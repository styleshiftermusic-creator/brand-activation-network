...
// Since we are running this as a TS script, we'll manually copy the courseData here 
// to avoid complex TS compiler path issues with Next.js aliases like `@/data/...`
const courseData = {
  "1": {
    ...
    "resources": [
      { "type": "PDF", "name": "Pledge Loan Setup Checklist", "url": "/resources/pledge-loan-checklist.pdf" },
      { "type": "PDF", "name": "Credit Union Contact List", "url": "/resources/cu-contact-list.pdf" },
      { "type": "DOC", "name": "Application Script Template", "url": "/resources/app-script.docx" }
    ],
    "quiz": [
      {
  ...
  "2": {
    ...
    "resources": [
      { "type": "PDF", "name": "Business Entity Setup Guide", "url": "/resources/entity-setup.pdf" },
      { "type": "XLS", "name": "Funding Stack Calculator", "url": "/resources/funding-calculator.xlsx" }
    ],
    "quiz": [
      {
...
async function seedModules() {
  console.log('Seeding course modules into Supabase...');

  const modulesToInsert = Object.entries(courseData).map(([key, data]) => {
    return {
      id: `M-0${key}`,
      title: data.title,
      category: data.category || 'Uncategorized',
      audio_src: data.audioSrc,
      visuals: (data as any).visuals || [],
      study_guide: (data as any).studyGuide,
      resources: (data as any).resources || [],
      quiz: (data as any).quiz || [],
      order_index: parseInt(key, 10)
    };
  });
