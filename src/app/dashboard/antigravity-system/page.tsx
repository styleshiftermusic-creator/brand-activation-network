...
export default function AntiGravitySystemPage() {
    return (
        <div className="min-h-screen bg-[var(--brand-bg)] flex text-zinc-300 font-sans selection:bg-[var(--primary)]/30 relative overflow-hidden">
            ...
            <div className="fixed top-0 left-1/4 w-[800px] h-[800px] bg-[var(--brand-primary)]/5 rounded-full blur-[150px] pointer-events-none z-0" />
            ...
            <main className="flex-1 p-6 lg:p-10 overflow-y-auto z-10 flex items-center justify-center relative">
                <div className="max-w-3xl w-full">
                    {/* Coming Soon Card */}
                    <div className="relative group rounded-3xl overflow-hidden border border-[var(--brand-primary)]/20 bg-black/40 backdrop-blur-2xl p-12 md:p-20 text-center flex flex-col items-center">
                        
                        {/* Shimmer Effect */}
                        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full group-hover:animate-shimmer pointer-events-none" />
                        
                        {/* Subtle Glow Background */}
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[var(--brand-primary)]/10 rounded-full blur-[100px] pointer-events-none" />
                        
                        <div className="relative z-10">
                            {/* Icon Assembly */}
                            <div className="relative w-24 h-24 mx-auto mb-8 flex items-center justify-center">
                                <Orbit className="w-full h-full text-[var(--brand-primary)]/20 absolute animate-[spin_10s_linear_infinite]" />
                                <Cpu className="w-12 h-12 text-[var(--brand-primary)] absolute" />
                                <div className="absolute -top-1 -right-1 bg-black rounded-full p-1.5 border border-[var(--brand-primary)]/30">
                                    <Lock className="w-4 h-4 text-indigo-300" />
                                </div>
                            </div>

                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[var(--brand-primary)]/10 border border-[var(--brand-primary)]/20 text-[var(--brand-primary)] text-xs font-mono tracking-[0.2em] uppercase mb-6">
                            ...
                            <h1 className="text-4xl md:text-6xl font-extrabold tracking-tighter text-white mb-6 uppercase">
                                The <span className="bg-clip-text text-transparent bg-gradient-to-r from-[var(--brand-primary)] to-[var(--brand-info)]">AntiGravity</span> System
                            </h1>
